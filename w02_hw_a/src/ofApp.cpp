...
void ofApp::setup(){
    ...
    for(int i=0; i< num; i++){
        ...
        changeTheta[i] = ofRandom(.01,0.01);
        changePhi[i] = ofRandom(.01,0.01);
...
void ofApp::draw(){
    ...
    for(int i = 0; i < num; i++){
        ...
        points[i] = ofPoint(radius[i]*sin(phi[i])*sin(theta[i]), radius[i]*cos(phi[i])*sin(theta[i]), radius[i]*sin(phi[i]));
